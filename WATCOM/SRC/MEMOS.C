 ...
 *				introduced use of TRUE and FALSE.
 * 91/02/14	Jack Schueler	For QNX, support different memo file name.
 */

/* Define TRUE and FALSE to make the code more readable.
 */
#define TRUE	1
#define FALSE	0
#if defined(__QNX__)
...
static const char FileName[] = { "memos" SUFFIX };
static const char TempName[] = { "tempmemo" SUFFIX };
...
/* This table maps action keywords onto the "actions" defined above.
 * The table also defines short forms for the keywords.
 */
...
/* Function prototypes generated by changing all static functions to extern
   and then recompiling with -v option which generates "memos.def".
 */
#include "memos.def"

static MEMO_EL *    ReadAMemo( FILE * __fid );
static TEXT_LINE *  AddLine( char __buf[], MEMO_EL * __el, TEXT_LINE * __pl );
static ACTION	    PromptAction( void );
static ACTION	    ReadAction( void );
static ACTION	    GetAction( char __buf[] );
static MEMO_EL *    DoActions( MEMO_EL * __el, ACTION __act );
static MEMO_EL *    AddMemo( MEMO_EL * __el );
static MEMO_EL *    EnterAMemo( void );
static MEMO_EL *    DeleteMemo( MEMO_EL * __el );
static MEMO_EL *    DoUpAction( MEMO_EL * __el );
static MEMO_EL *    DoDownAction( MEMO_EL * __el );
static MEMO_EL *    ShowTodaysMemos( void );
static void *	    MemoMAlloc( int __size );
...
/* Use the command line parameters, if any, as the first actions to
 * be performed on the memos.
 */
    el = NULL;
 ...
 */
{
    ...
    if( fid == NULL ) {
	printf( "Memos file not found. Starting with no memos.\n" );
	return;
    }
...
/* Read a line from the memos file. Handle any I/O errors and EOF.
 * Return the length read, not counting the newline on the end.
 */
{
...
/* Read one memo, creating the memo structure and filling it in.
 * Return a pointer to the memo (NULL if none read).
 */
{
    MEMO_EL *	el;
    int 	len;
...
/* Add a line of text to the memo, taking care of all the details
 * of modifying the structure.
 */
{
    TEXT_LINE * line;
...
/* Read a memo from the keyboard, creating the memo structure and filling it in.
 * Return a pointer to the memo (NULL if none read).
 */
{
    MEMO_EL *	el;
 ...
 */
	...
	if( el->date[0] == '\0'  ||  strcmp( date, el->date ) == 0 ) {
	    DisplayMemo( el );
	    last_el = el;
	}
    }
